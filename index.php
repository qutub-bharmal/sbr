...
$shopifyResponse = json_decode($json, true);
   ...
   // if ($order) {
			foreach( $shopifyResponse['fulfillments'] as $fulfillments ) {
				foreach( $fulfillments['line_items'] as $item ) {
					$spb_item = $class->searchItem($item['sku']);

					if( isset($spb_item['total']) && $spb_item['total'] == 1 ) {
						/*$item_ids[] = [
						...
						$item_data = [
							//'item_id' => $spb_item['results'][0]['id'],
              				'item_lookup' => '"'.$spb_item['results'][0]['public_id'].'"',
							'qty' => intval($item['quantity']),
							'ship_from_location_id' => intval(100005),
						];

						//$d = $class->addItemToOrder($order, $item_data);
					}
				}
		//	}
     ...
     // $order_data['lines'] = $item_data;
      ...
      echo "</pre>";exit;
			...
			$d = $class->createPayment($order, $payment_data);

			$d = $class->changeOrderStatus($order, "open");
			...
			$d = $class->addItemToInvoice($invoice, $item_ids);
			$d = $class->updateInvoice($invoice, ['status' => 'complete']);
